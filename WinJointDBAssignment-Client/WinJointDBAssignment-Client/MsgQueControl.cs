...
namespace WinJointDBAssignment_Client
{
    ...
    public class MsgQueControl
    {
        ...
        const string MY_QUEUE_NAME = "User";                                        ///< my queue name (predetermined)
        ...
        TimeSpan timeSpanRev;                                                       ///< The Timespan we're waiting for
        ...
        public MsgQueControl()
        {
            ...
            this.timeSpanRev = new TimeSpan(0, 0, 2);   // time span is 2 seconds when receive 
        ...
        public CommandData Connect(string serverMachineName, string userType, string userName)  // userType: "User" or "Admin"
        {
            ...
            connMsg.isRunning = true;            ////////////////////////// added for user
            connMsg.qName = this.myMessageQ.Path;
           
            tmpMQ.Send(connMsg); // send connect request to server
            CommandData serverMsg = null;
            serverMsg = this.Receive(CommandData.LEADERBOARD);   // wait to get acceptace from server

           // serverMsg = this.Receive('Z');   // wait to get acceptace from server
